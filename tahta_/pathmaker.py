...
class MapManager:
    ...
    def draw(self, surf):
        if self.mode != "room_type":
            ...
            for node in self.posmap:
                drawcircle(surf, node["pos"], "blue", (self.x, self.y))
                if len(self.map[i]) != 0:
                    for neighbourid in self.map[i]:
                        drawline(
                            surf, self.posmap[i]["pos"], self.posmap[neighbourid]["pos"], "green", (self.x, self.y))
                surf.blit(self.app.font.render(str(i), False, (240, 240, 240)),
                          (node["pos"][0]-self.x, node["pos"][1]-self.y))
                i += 1
        else:
            i = 0
            for node in self.posmap:
                col = ROOM_TYPE_COLORS[self.typemap[i]]
                drawcircle(surf, node["pos"], col, (self.x, self.y))
                if len(self.map[i]) != 0:
                    for neighbourid in self.map[i]:
                        drawline(
                            surf, self.posmap[i]["pos"], self.posmap[neighbourid]["pos"], "green", (self.x, self.y))
    ...
    def convert(self):
        ...
        for i, node in enumerate(self.map):
            exportmap.append({"neighbors": node, "type": self.typemap[i]})

        return exportmap # self.map
